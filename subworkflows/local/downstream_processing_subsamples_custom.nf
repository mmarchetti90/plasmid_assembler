...
include { MedakaPolishConsensus } from '../../modules/local/medaka/medaka_polish_consensus.nf'
...
workflow DOWNSTREAM_PROCESSING_SUBSAMPLES_CUSTOM {
  ...
  concatenated_cleaned_fastq
    .join(consensus_assembly, by: 0, remainder: false)
    .set{ medaka_input }

  // Polish assembly
  MedakaPolishConsensus(medaka_input)

  polished_assembly = MedakaPolishConsensus.out.polished_assembly
  draft_alignment = MedakaPolishConsensus.out.draft_alignment
  ...
  // ab1 generation
  PileupToAb1(alignment_script, "custom_consensus_assembly", alignment_pileup)
